...
<body>
	...
	<div class="container" style="text-align: center; padding-bottom: 4px;">
		<h1 style="font-family: Monaco;">Calculate Fare</h1>

	</div>
	<br />

	<div style="float: right; margin-bottom: 2em;">
		<f:form action="calculatefare" method="post" class="form-inline"
			modelAttribute="searchbean">
			<label class="my-1 mr-2" style="margin-left: 1em; margin-right: 1em;">Boarding</label>
			<f:select path="boarding" list="stops" items="${allstop}"
				name="boarding" class="custom-select my-1 mr-sm-2"
				style="margin-left: 1em;margin-right: 1em;">
			</f:select>

			<label class="my-1 mr-2" style="margin-left: 1em; margin-right: 1em;">Destination</label>

			<f:select path="destination" list="stops" items="${allstop}"
				name="destination" class="custom-select my-1 mr-sm-2"
				style="margin-left: 1em;margin-right: 1em;">
			</f:select>
						...
						<table class="table table-hover">
							...
							<tbody>
								<c:forEach items="${locbus}" var="locbus">
									...
									<tr>
										<th scope="row"><c:out value="${locbus.bus_no}" /></th>
										<td><c:out value="${locbus.currentLocation}" /></td>
										<td><c:out value="${locbus.fare}" /></td>
										<td><a class="btn btn-primary"
											href="/booktickets?bus_no=${locbus.bus_no}&fare=${locbus.fare}&pickup=${searchbean.boarding}&drop=${searchbean.destination}"
											role="button"> BookNow </a></td>

										<td><a class="btn btn-primary"
											href="/notify?bus_no=${locbus.bus_no}&pickup=${searchbean.boarding}&droping=${searchbean.destination}"
											role="button"> Notify </a></td>
